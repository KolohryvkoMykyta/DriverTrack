...
namespace DriverTrack.Infrastructure.Repositories
{
    public sealed class RouteRepository : IRouteRepository
    {
        ...
        public Task<List<RouteEntry>> GetByDriverIdWithDateFilterAsync(Guid driverId, DateTime? from = null, DateTime? to = null, CancellationToken cancellationToken = default)
        {
            var query = _dbContext.RouteEntries
                .AsNoTracking()
                .Where(re => re.DriverId == driverId);
